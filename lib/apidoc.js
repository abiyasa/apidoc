...
function createOutputFiles(parsedFiles, parsedFilenames, packageInfos)
{
	...
	if ( options.outputToMarkdown ) {  // write to markdown
		var Handlebars = require("handlebars");
		var registerHelpers = require("./utils/handlebars_helpers")(Handlebars);

		// prepare template
		var defaultTemplateDir = path.join(__dirname, "../template/");
		if (options.template === defaultTemplateDir) {
			// use default hbs template file
			options.template = path.join(__dirname, "../template_markdown/basic.hbs");
		}
		app.debug("reading markdown template file: " + options.template);
		var hbsTemplate = fs.readFileSync(options.template, "utf8");
		var compiledTemplate = hbsTemplate ? Handlebars.compile(hbsTemplate) : undefined;

		// filter data, only show the latest version
		var latestVersion = packageInfos.version;
		app.debug("filtering to only the latest version: " + latestVersion);
		blocks = blocks.filter(function(block) {
			return (block.version === "0.0.0") || block.version === latestVersion;
		});

		// prepare data
		var docData = {
			project: packageInfos,
			data: blocks
		};

		// write to file
		var targetMarkdownFile = options.dest + options.outputToMarkdown;
		app.debug("write markdown file: " + targetMarkdownFile);
		if( ! options.simulate) fs.writeFileSync(targetMarkdownFile, compiledTemplate(docData));

		// api_data
		var json = JSON.stringify(docData, null, 2);
...
function main(defaults)
{
	...
	// handle template
	if (!options.outputToMarkdown) {
		// template param is a folder (instead of markdown file)
